...
export function MedicalSecuritySection({ personalInfo }: MedicalSecuritySectionProps) {
  ...
  const formatEpsDisplay = (eps?: string, epsOther?: string) => {
    if (!eps) return t('messages.notSpecified');
  ...
  const formatComplementaryPlanDisplay = (plan?: string, planOther?: string) => {
    if (!plan) return t('messages.notSpecified');
  ...
  return (
    <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-150">
      <div className="flex items-center mb-3">
        ...
        <h4 className="text-sm font-semibold text-vitalgo-dark ml-2">{t('sectionTitles.socialSecurityOccupation')}</h4>
      </div>

      <div className="space-y-3">
        {/* EPS */}
        <div className="flex justify-between items-center">
          <span className="text-xs text-vitalgo-dark-lighter">{t('labels.eps')}</span>
        ...
        {/* Occupation */}
        <div className="flex justify-between items-center">
          <span className="text-xs text-vitalgo-dark-lighter">{t('labels.occupation')}</span>
          <span className="text-sm font-medium text-vitalgo-dark">
            {personalInfo?.occupation || t('messages.notSpecified')}
          </span>
        </div>
        ...
        {personalInfo?.additional_insurance && (
          <div className="flex justify-between items-center">
            <span className="text-xs text-vitalgo-dark-lighter">{t('labels.additionalInsurance')}</span>
            <span className="text-sm font-medium text-vitalgo-dark">
              {personalInfo.additional_insurance}
            </span>
          </div>
        ...
        {personalInfo?.complementary_plan && (
          <div className="flex justify-between items-center">
            <span className="text-xs text-vitalgo-dark-lighter">{t('labels.complementaryPlan')}</span>
            <span className="text-sm font-medium text-vitalgo-dark">
