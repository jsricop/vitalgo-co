...
export function GynecologicalInformationTab({ 'data-testid': testId }: TabContentProps) {
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">{t('notApplicable')}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const getPregnancyStatusLabel = (isPregnant: boolean | null | undefined) => {
    if (isPregnant === null || isPregnant === undefined) return t('notSpecified');
    return isPregnant ? tCommon('yes') : tCommon('no');
  };

  const getMenstrualStatusLabel = (status: string | null | undefined) => {
    if (!status) return t('notSpecified');
    const statusMap: Record<string, string> = {
      'NOT_STARTED': t('menstrualStatus.notStarted'),
      'ACTIVE': t('menstrualStatus.active'),
      'MENOPAUSE': t('menstrualStatus.menopause'),
    };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-150">
              <div className="flex items-center mb-3">
                ...
                <h4 className="text-sm font-semibold text-vitalgo-dark ml-2">{t('cards.pregnancy.title')}</h4>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-vitalgo-dark-lighter">{t('cards.pregnancy.isPregnant')}</span>
                ...
                {gynecologicalData.is_pregnant && (
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-vitalgo-dark-lighter">{t('cards.pregnancy.weeks')}</span>
                    <span className="text-sm font-medium text-vitalgo-dark">{gynecologicalData.pregnancy_weeks || t('notSpecified')}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-150">
              <div className="flex items-center mb-3">
                ...
                <h4 className="text-sm font-semibold text-vitalgo-dark ml-2">{t('cards.menstrual.title')}</h4>
              </div>
              <div className="space-y-2">
                <div>
                  <span className="text-xs text-vitalgo-dark-lighter block">{t('cards.menstrual.status')}</span>
                  <span className="text-sm font-medium text-vitalgo-dark">{getMenstrualStatusLabel(gynecologicalData.menstrual_status)}</span>
                </div>
                <div>
                  <span className="text-xs text-vitalgo-dark-lighter block">{t('cards.menstrual.lastPeriod')}</span>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-150">
              <div className="flex items-center mb-3">
                ...
                <h4 className="text-sm font-semibold text-vitalgo-dark ml-2">{t('cards.reproductive.title')}</h4>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-vitalgo-dark-lighter">{t('cards.reproductive.pregnancies')}</span>
                  <span className="text-sm font-medium text-vitalgo-dark">{gynecologicalData.pregnancies_count ?? 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-vitalgo-dark-lighter">{t('cards.reproductive.births')}</span>
                  <span className="text-sm font-medium text-vitalgo-dark">{gynecologicalData.births_count ?? 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-vitalgo-dark-lighter">{t('cards.reproductive.cesareans')}</span>
                  <span className="text-sm font-medium text-vitalgo-dark">{gynecologicalData.cesareans_count ?? 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-vitalgo-dark-lighter">{t('cards.reproductive.abortions')}</span>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-150">
              <div className="flex items-center mb-3">
                ...
                <h4 className="text-sm font-semibold text-vitalgo-dark ml-2">{t('cards.contraceptive.title')}</h4>
              </div>
              <div className="space-y-2">
                <div>
                  <span className="text-xs text-vitalgo-dark-lighter block">{t('cards.contraceptive.currentMethod')}</span>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            <div className="flex items-center">
              ...
              <span className="text-sm text-vitalgo-dark-lighter">
                {isComplete ? t('status.available') : t('status.incomplete')}
              </span>
            </div>
